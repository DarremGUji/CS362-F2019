...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    case mine:
    	return mineCardEffect(choice1, choice2, state, handPos);
    ...
    case minion:
        //+1 action
        return minionCardEffect(choice1,choice2,state,handPos);
    ...
    case tribute:
    	return tributeCardEffect(state, handPos);
...
int baronCardEffect(int discard, struct gameState *state, int currentPlayer){
...
int minionCardEffect(int choice1, int choice2, struct gameState *state, int handPos){
		...
		if (choice1)
        {
            //state->coins = state->coins + 2; add bug3
        }
        else if (choice2)		//discard hand, redraw 4, other players with 5+ cards discard hand and draw 4
        {
            ...
            //draw 4
            for (i = 0; i < 3; i++)//change 4 to 3, add bug 4
            {
                drawCard(currentPlayer, state);
            }
...
int tributeCardEffect(struct gameState *state, int handPos){
        ...
        for (i = 0; i <= 2; i ++) {
            if (tributeRevealedCards[i] == copper || tributeRevealedCards[i] == silver || tributeRevealedCards[i] == gold) { //Treasure cards
               ...
               // state->numActions = state->numActions + 2; //bug added 8
            }
        }
...
int mineCardEffect(int choice1, int choice2, struct gameState *state, int handPos){
        ...
        if ( (getCost(state->hand[currentPlayer][choice1]) ) > getCost(choice2) )//bug added 9
        {
            return -1;
        }
        ...
        //discard card from hand
        //discardCard(handPos, currentPlayer, state, 0); bug added 10
